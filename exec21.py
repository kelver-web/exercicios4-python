# 21. Faça um programa que carregue uma lista com os modelos de
...
# combustível. Calcule e mostre: O modelo do carro mais econômico;
...
isto custará, considerando um que a gasolina custe R$ 2,25 o litro.
Abaixo segue uma tela de exemplo. O disposição das informações deve
ser o mais próxima possível ao exemplo. Os dados são fictícios e
...
espaco1 = '                     '
espaco2 = ' '
print('VEÍCULOS', espaco1, 'CONSUMO', espaco2, 'KM/L', espaco1, 'VALOR')

for i in range(5):
    print(f'{i + 1} - {veiculos[i]}    - {consumo[i]}\
     - {1000 / consumo[i]:.2f}\
     - R$ {(1000 / consumo[i] * 2.252):.2f}')
menor_consumo = consumo.index(max(consumo))
print(f'O menor consumo é o do {veiculos[menor_consumo]}')
