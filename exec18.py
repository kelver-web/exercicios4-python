# 18. Uma grande emissora de televisão quer fazer uma enquete entre
...
# programação C++. Para computar cada voto, a telefonista digitará um número,
# entre 1 e 23, correspondente ao número da camisa do jogador. Um número de
# jogador igual zero, indica que a votação foi encerrada. Se um número inválido
# for digitado, o programa deve ignorá-lo, mostrando uma breve mensagem
# de aviso,
# e voltando a pedir outro número. Após o final da votação, o programa deverá
# exibir:
...
de uma função. Esta função receberá dois parâmetros: o número de votos de um jogador
...
for i in range(23):
    if contador not in votos:
        ...
        print(f'\nJogador camisa n° ---> {contador} = {votos.count(contador)} Votos')
        print(f'Percentual de votos: {100 * votos.count(contador) / len(votos):.2f} %')
        contador += 1
melhor = votos_jogadores_votados.index(max(votos_jogadores_votados))
print(f'\nJogador mais votado n° ----> {numeros_jogadores_votados[melhor]} com {votos_jogadores_votados[melhor]} Votos')
print(f'Vencedor com ----> {100 * votos_jogadores_votados[melhor] / len(votos):.2f} % dos votos')
